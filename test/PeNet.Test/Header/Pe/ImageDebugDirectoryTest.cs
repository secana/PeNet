...
namespace PeNet.Test.Header.Pe
{
    ...
    public class ImageDebugDirectoryTest
    {
        ...
        public void ParseDebugDirectory_MultipleEntries_ReturnsAllEntries()
        {
            ...
            Assert.Equal(DebugDirectoryType.CodeView, debugEntries[0].DebugType);
            Assert.Equal(DebugDirectoryType.VcFeature, debugEntries[1].DebugType);
            Assert.Equal(DebugDirectoryType.Pogo, debugEntries[2].DebugType);
            Assert.Equal(DebugDirectoryType.Iltcg, debugEntries[3].DebugType);
        }

        [Fact]
        public void ParseDebugDirectory_MultipleEntries_ExtendedDllCharacteristics()
        {
            var file = @"Binaries/TestCetCompatAndEhCont.exe";

            var debugEntries = new PeFile(file).ImageDebugDirectory;

            Assert.Equal(5, debugEntries.Length);
            Assert.Equal(DebugDirectoryType.CodeView, debugEntries[0].DebugType);
            Assert.Equal(DebugDirectoryType.VcFeature, debugEntries[1].DebugType);
            Assert.Equal(DebugDirectoryType.Pogo, debugEntries[2].DebugType);
            Assert.Equal(DebugDirectoryType.Iltcg, debugEntries[3].DebugType);
            Assert.Equal(DebugDirectoryType.ExtendedDllCharacteristics, debugEntries[4].DebugType);
            Assert.Equal(ExtendedDllCharacteristicsType.CetCompat, debugEntries[4].ExtendedDllCharacteristics);
        }
